...
from .dnn_file_generator import generate_class_file
...
class DNNGenerator:
    ...
    def save_models(self, models):
        ...
        if models is None:
            ...
            self.__save_model(models-1)
        elif isinstance(models, list):
            for i in models:
                self.__save_model(int(i) - 1)
    
    def __save_model(self, index):
        try:
            path = os.path.join(self.dir_path, "model-"+str(index+1))
            self.models[index].save(path)
            print("[INFO] Saved model %d to %s" % (index+1, path))
        except:
